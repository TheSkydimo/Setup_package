...
# 脚本所在目录及项目根目录（脚本在 `<项目根>/APEX_Pack` 下）
...
def sign_file(file_path, timestamp_url, digest_algorithm):
    ...
    command = [
        ...
        "/d", "APEX Setup — PC Ambient Lighting Controller",
        "/du", " ",
        full_file_path
    ]
...
def process_package(current_dir, timestamp_urls):
    ...
    # 配置不同版本的参数
    config = {
        # 可执行文件位于项目根目录下的 APEX 目录
        'exefullname': os.path.join(PROJECT_ROOT, "APEX", "APEX.exe"),
        'OEMVer': 1,
        'Ver_value': 1,
        'project_name': "APEX",
        # Inno Setup 脚本位于项目根目录下的 iss 目录
        'iss_file': os.path.join(PROJECT_ROOT, "iss", "APEX.iss"),
        'setup_filename': "APEXSetup.exe"
    }
    ...
    # 获取版本号：
    #   - file_version_raw    : APEX.exe 的 FileVersion（属性页中的“文件版本”）
    #   - product_version_raw : APEX.exe 的 ProductVersion（属性页中的“产品版本”）
