...
; 下方三个版本宏由 `ApexBSignBat.py` 自动维护（请勿手动修改）：
;   MyAppVersion     : 安装器显示用的版本号（优先完整 ProductVersion，例如 2.0.2.r251201）
;   MyFileVersion    : 可执行文件的 FileVersion（例如 2.0.2.0）
;   MyProductVersion : 可执行文件的 ProductVersion（例如 2.0.2.r251201）
#define AppVersionFull "2.0.3.e4f79ca"
...
; 打开日志功能
SetupLogging=yes
OutputBaseFilename=APEXSetup_{#AppVersionFull}
...
//-----------------------------
//通用语言自动选择（所有语言）
//- 尝试多种 Windows 语言来源：UI / UserDefaultUI / Locale
//- 只在你的 [Languages] 列表包含该语言时才命中
//- 未命中 -> 回退 english
//-----------------------------
//Windows API (kernel32). Inno Setup Pascal Script does not provide this built-in.
function GetUserDefaultUILanguage(): Integer;
  external 'GetUserDefaultUILanguage@kernel32.dll stdcall';

function GetUserDefaultLangID(): Integer;
  external 'GetUserDefaultLangID@kernel32.dll stdcall';

function PrimaryLangID(LangID: Integer): Integer;
begin
  Result := LangID and $03FF;
end;

function SubLangID(LangID: Integer): Integer;
begin
  Result := (LangID shr 10) and $003F;
end;

function LangIDToInnoLanguageName(LangID: Integer): string;
var
  P: Integer;
begin
  Result := '';
  P := PrimaryLangID(LangID);
  ...
  case P of
    ...
    $11: Result := 'japanese';
    $12: Result := 'korean';
    $16: Result := 'portuguese';
    $19: Result := 'russian';
    $1F: Result := 'turkish';
    $2A: Result := 'vietnamese';

    $04:
      begin
        //中文：仅提供简中则只命中简中，其它（繁中/港澳台）按“未列出语言”处理 -> 回退英文
        if SubLangID(LangID) = $02 then
...
function PickBestLanguage(): string;
var
  L: Integer;
  Name: string;
begin
  //1) Windows UI 语言
  L := GetUILanguage;
  Name := LangIDToInnoLanguageName(L);
  if Name <> '' then begin Result := Name;Exit;end;
...
var
  Chosen: string;
begin
  //设置默认语言（仍会弹语言选择框，但默认项会选中此语言）
  Chosen := PickBestLanguage();
  //SetActiveLanguage(Chosen);

  //可选日志：用于现场排查（保留也无妨）
  Log(Format('Language pick: %s', [Chosen]));
  Log(Format('GetUILanguage=%x', [GetUILanguage]));
  Log(Format('GetUserDefaultUILanguage=%x', [GetUserDefaultUILanguage]));
  Log(Format('GetUserDefaultLangID=%x', [GetUserDefaultLangID]));
